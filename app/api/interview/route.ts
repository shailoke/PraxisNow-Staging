...
export async function POST(request: NextRequest) {
    try {
        ...
        const isStreamMode = request.headers.get('x-response-mode') === 'stream'
        ...
        try {
            // =========================================================
            // STREAMING PATH — triggered by x-response-mode: stream
            // =========================================================
            if (isStreamMode) {
                ...
                const readable = new ReadableStream({
                    async start(controller) {
                        try {
                            // STEP 1 (stream): Generate AI response with streaming
                            let streamedText = ''
                            ...
                            for await (const chunk of stream1) {
                                ...
                                if (delta?.tool_calls) {
                                    ...
                                    if (!pendingToolCall) pendingToolCall = { id: '', name: '', arguments: '' }
                            ...
                            // Handle tool call (interview_time_status)
                            if (pendingToolCall?.name === 'interview_time_status') {
                                ...
                                const timeStatus = { elapsed_minutes: Math.floor(elapsed), remaining_minutes: remaining }

                                console.log(`⏱️ [TOOL_EXEC_STREAM] Time Status: ${timeStatus.elapsed_minutes}m elapsed, ${timeStatus.remaining_minutes}m remaining`)

                                openaiMessages.push({
                                    role: 'assistant',
                                    tool_calls: [{ id: pendingToolCall.id, type: 'function', function: { name: pendingToolCall.name, arguments: pendingToolCall.arguments } }]
                                } as any)
                                openaiMessages.push({
                                    role: 'tool',
                            ...
                            const streamAssistantMessage = streamedText.trim()

                            // Content validation
                            if (!streamAssistantMessage || streamAssistantMessage.length === 0) {
                                throw new Error('Generated empty question content')
                            }
                            if (streamAssistantMessage.includes('Candidate:') || streamAssistantMessage.startsWith('Candidate:')) {
                                console.error('❌ [INVARIANT_VIOLATION] System generated candidate speech (stream)')
                                throw new Error('Invariant violation: System generated candidate speech.')
                            }

                            console.log('[TURN_GENERATION_SUCCESS_STREAM]', { session_id })

                            // DB writes — identical to non-streaming path
                                ...
                                try {
                                    if (turn_authority === true) {
                                        ...
                                        if (latestTurns && latestTurns.length > 0) {
                                            const { error: updateError } = await supabaseClient
                                                ...
                                                .eq('id', latestTurns[0].id)

                                            if (updateError) {
                                                console.error('❌ [PERSISTENCE_FAILURE_STREAM] Failed to mark turn answered:', updateError)
                                            } else {
                                                console.log(`✅ [TURN_ANSWERED_STREAM] Marked turn #${latestTurns[0].turn_index} as answered`)
                                            }
                                        }
                                    }
                                    ...
                                    const { data: insertedTurn, error: insertError } = await supabaseClient
                                        ...
                                        .insert({
                                            ...
                                            content: streamAssistantMessage,
                                            answered: false,
                                    ...
                                    if (insertError) {
                                        console.error('⚠️ [STREAM] Failed to insert interview_turn:', insertError)
                                    } else {
                                        console.log(`✅ [TURN_CREATED_STREAM] Turn #${nextIndex} created (id: ${(insertedTurn as any)?.id})`)
                                    }

                                    // Fire-and-forget question history (replay guard)
                                        ...
                                        try {
                                            ; (supabaseClient as any)
                                                ...
                                                .insert({
                                                    ...
                                                    session_id,
                                                    turn_index: nextIndex,
                                                    question_text: streamAssistantMessage
                                                } as any)
                                                .then(({ error: historyError }: { error: any }) => {
                                                    if (historyError) console.warn('[QUESTION_HISTORY_STREAM] Non-blocking insert failed:', historyError)
                                                })
                                        } catch (historyInitErr) {
                                            console.warn('[QUESTION_HISTORY_STREAM] Insert threw synchronously:', historyInitErr)
                                        }
                                    }
                                } catch (dbError) {
                                    console.error('[STREAM] DB error (non-fatal, content already streamed):', dbError)
                                }
                            }

                            controller.close()
                        } catch (streamErr) {
                            console.error('[STREAM] Fatal error in stream handler:', streamErr)
